...
class Led_strip:
    ...
    def __init__(self, pin=board.D10, num_pixels=80, brightness=1):
    ...
    def stepstone(self, section_size):
        num_pixels = len(self.pixels)

        for i in range(0, num_pixels, section_size):
            j = min(i + section_size, num_pixels)

            for brightness in range(256):
                section_brightness = tuple(
                    int(val * brightness / 255) for val in self.color_array[i % len(self.color_array)]
                )
                self.pixels[i:j] = [section_brightness] * (j - i)
                time.sleep(0.01)  # Adjust the sleep duration for a smoother transition

        time.sleep(3)  # Add an additional delay between sections if needed
