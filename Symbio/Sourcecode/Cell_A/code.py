...
hub_A_fan_in = pwmio.PWMOut(board.D13, frequency=5000, duty_cycle=0)
hub_A_fan_out = pwmio.PWMOut(board.D10, frequency=5000, duty_cycle=0)
hub_A_fan_mist = pwmio.PWMOut(board.D7, frequency=5000, duty_cycle=0)
curState = 0
...
def crowded():
    ...
    if time.monotonic() - now > 9:
        mqtt_client.publish("Group-1/Activity", 1) # Send an activity update
        curState = 0
    elif time.monotonic() - now > 7.5:
#         adjust_fans(0, 0, 0.1, hub_A_fan_in, hub_A_fan_out, hub_A_fan_mist)
        adjust_fans(0, 0, 1, hub_A_fan_in, hub_A_fan_out, hub_A_fan_mist)

    elif time.monotonic() - now > 5:
        adjust_fans(0, 0.6, 0, hub_A_fan_in, hub_A_fan_out, hub_A_fan_mist)
    elif time.monotonic() - now > 3:
        adjust_fans(0, 0, 0, hub_A_fan_in, hub_A_fan_out, hub_A_fan_mist)
    else:
        adjust_fans(1, 0, 0, hub_A_fan_in, hub_A_fan_out, hub_A_fan_mist)

def busy():
    global curState, now
    if time.monotonic() - now > 16:
        mqtt_client.publish("Group-1/Activity", 1) # Send an activity update
        curState = 0
    elif time.monotonic() - now > 14:
#         adjust_fans(0, 0, 0.03, hub_A_fan_in, hub_A_fan_out, hub_A_fan_mist)
        adjust_fans(0, 0, 1, hub_A_fan_in, hub_A_fan_out, hub_A_fan_mist)
    elif time.monotonic() - now > 8:
        adjust_fans(0, 0.3, 0, hub_A_fan_in, hub_A_fan_out, hub_A_fan_mist)
    elif time.monotonic() - now > 3:
        adjust_fans(0, 0, 0, hub_A_fan_in, hub_A_fan_out, hub_A_fan_mist)
    else:
        adjust_fans(0.4, 0, 0, hub_A_fan_in, hub_A_fan_out, hub_A_fan_mist)
...
def quiet():
    ...
    if time.monotonic() - now > 22:
        ...
        adjust_fans(0, 0.1, 0, hub_A_fan_in, hub_A_fan_out, hub_A_fan_mist)
    elif time.monotonic() - now > 4:
        adjust_fans(0, 0, 0, hub_A_fan_in, hub_A_fan_out, hub_A_fan_mist)
    else:
        adjust_fans(0.3, 0, 0, hub_A_fan_in, hub_A_fan_out, hub_A_fan_mist)
